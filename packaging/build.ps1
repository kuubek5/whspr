# build.ps1 -- freeze KuubWave and package the installer. Run from the repo root:
...
# step silently skipped, leaving dist\KuubWave but no setup.exe.
$isccCandidates = @(
...
if (-not $iscc) {
    ...
    Write-Warning "  & `"$iscc`" packaging\kuubwave.iss"
    Write-Host "Frozen app is ready in dist\KuubWave" -ForegroundColor Green
    exit 0
}
